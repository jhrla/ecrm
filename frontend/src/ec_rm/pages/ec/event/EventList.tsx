...
import EventListTable from './table/EventListTable';
import EventListSearchArea, {
  SearchParams,
} from './searchbar/EventListSearchArea';
...
export const EventList = (): JSX.Element => {
  ...
  useEffect(() => {
    fetchEventList(searchParams, currentPage, pageSize);
  ...
  const handleExcelDownload = () => {
    try {
      downloadExcel(
        '/api/downloadEventList',
        searchParams,
        '경보/주의보 이력_' + new Date().toISOString().split('T')[0]
      ).catch((error) => {
  ...
  return (
    <div className='inner'>
      <EventListSearchArea onSearch={handleSearch} />
      {/* 검색 결과 테이블 영역 */}
      <div className='white_box'>
        ...
        <EventListTable eventList={eventList} />
